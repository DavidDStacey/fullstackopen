...
const Blog = ({blog, blogs, setBlogs }) => {
  ...
  return (
    ...
    <div style={blogStyle}>
      ...
      {toggle ? <> by {blog.author} at {blog.url} likes: {blog.likes} <button onClick={(event) => updateLikeCount(event, blog.id)}>like</button><button onClick={(event) => deleteClick(event, blog.id)}>delete</button></> : <></>}
    </div>
    </>
  )
}
